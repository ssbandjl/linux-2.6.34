...
#define SWRITE_SYNC_PLUG (SWRITE | (1 << BIO_RW_SYNCIO) | (1 << BIO_RW_NOIDLE))
...
typedef void(dio_iodone_t)(struct kiocb *iocb, loff_t offset, ssize_t bytes,
                           void *private);
...
struct inode {
  ...
  struct list_head i_list; /* backing dev IO list 将inode存储到链表中, 3种状态*/
  ...
  const struct inode_operations
      *i_op; /* 函数指针集合, 抽象对inode的操作,由具体的文件系统实现, inode操作,
                如删除文件, 文件属性操作等 */
  const struct file_operations
      *i_fop; /* former ->i_op->default_file_ops   文件操作, 如操作文件数据 */
...
struct file {
  ...
  struct path f_path; // 文件名和inode关联
#define f_dentry f_path.dentry
#define f_vfsmnt f_path.mnt
  const struct file_operations *f_op; // 文件操作
  spinlock_t f_lock;                  /* f_ep_links, f_flags, no IRQ */
  atomic_long_t f_count;
  unsigned int f_flags; // open系统调用提供的额外参数
  fmode_t f_mode;       //打开文件传递的模式参数,如:读,写,读写
  loff_t f_pos; // 文件位置指针的当前值, 顺序读或读特定部分
  struct fown_struct f_owner; // 文件进程相关的信息
  const struct cred *f_cred;
  struct file_ra_state f_ra; //预读数据,提高性能
...
static inline int file_check_writeable(struct file *filp) { return 0; }
...
#if BITS_PER_LONG == 32
static inline int fcntl_getlk64(struct file *file,
...
static inline int fcntl_getlease(struct file *filp) { return 0; }

static inline void locks_init_lock(struct file_lock *fl) { return; }

static inline void __locks_copy_lock(struct file_lock *new,
                                     struct file_lock *fl) {
  return;
}
...
static inline void locks_remove_flock(struct file *filp) { return; }
...
static inline int posix_lock_file_wait(struct file *filp,
                                       struct file_lock *fl) {
  return -ENOLCK;
}
...
struct file_operations {
  ...
  unsigned int (*poll)(
      struct file *,
      struct poll_table_struct *); // poll和select系统调用, io多路复用
  int (*ioctl)(struct inode *, struct file *, unsigned int,
               unsigned long); // 与硬件设备通信
  long (*unlocked_ioctl)(struct file *, unsigned int, unsigned long);
  long (*compat_ioctl)(struct file *, unsigned int, unsigned long);
  int (*mmap)(struct file *, struct vm_area_struct *);
  int (*open)(struct inode *,
              struct file *); // 打开文件, 将file对象关联到一个inode
  int (*flush)(struct file *, fl_owner_t id);
  int (*release)(struct inode *,
                 struct file *); // file使用计数为0时,执行内存和缓存释放
  int (*fsync)(struct file *, struct dentry *, int datasync); // 内存 -> 存储
...
struct inode_operations {
  ...
  struct dentry *(*lookup)(struct inode *, struct dentry *,
                           struct nameidata *); //根据名称查找inode实例
  ...
  int (*rename)(struct inode *, struct dentry *, //文件重命名
                struct inode *, struct dentry *);
  int (*readlink)(struct dentry *, char __user *, int);
  void *(*follow_link)(struct dentry *,
                       struct nameidata *); // 通过符号链接查找目标文件inode
  void (*put_link)(struct dentry *, struct nameidata *, void *);
  void (*truncate)(struct inode *); //修改inode长度, 提前设置好i_size
  int (*permission)(struct inode *, int);
  int (*check_acl)(struct inode *, int);
  int (*setattr)(struct dentry *, struct iattr *);
  int (*getattr)(struct vfsmount *mnt, struct dentry *, struct kstat *);
  int (*setxattr)(struct dentry *, const char *, const void *, size_t,
                  int); // xattr建立,读取, 删除文件的扩展属性, 如acl
  ssize_t (*getxattr)(struct dentry *, const char *, void *, size_t);
  ssize_t (*listxattr)(struct dentry *, char *, size_t);
  int (*removexattr)(struct dentry *, const char *);
  void (*truncate_range)(
      struct inode *, loff_t,
      loff_t); //截断一个范围的块, 文件中穿孔, 一般用于共享文件系统
  long (*fallocate)(struct inode *inode, int mode,
                    loff_t offset, //对文件预先分配空间,提高性能, 如ext4
                    loff_t len);
 ...
 * on the filesystem.
 */
static inline void drop_nlink(struct inode *inode) { inode->i_nlink--; }
...
extern int get_sb_ns(struct file_system_type *fs_type, int flags, void *data,
                     ...
                     struct vfsmount *mnt);
extern int get_sb_bdev(struct file_system_type *fs_type, int flags,
                       const char *dev_name, void *data,
                       int (*fill_super)(struct super_block *, void *, int),
                       struct vfsmount *mnt);
extern int get_sb_single(struct file_system_type *fs_type, int flags,
                         void *data,
                         int (*fill_super)(struct super_block *, void *, int),
                         struct vfsmount *mnt);
...
extern struct file *do_filp_open(int dfd, const char *pathname, int open_flag,
...
#else
static inline int xip_truncate_page(struct address_space *mapping,
                                    loff_t from) {
  return 0;
}
#endif
...
blockdev_direct_IO(int rw, struct kiocb *iocb, struct inode *inode,
  ...
  return __blockdev_direct_IO(rw, iocb, inode, bdev, iov, offset, nr_segs,
                              get_block, end_io, DIO_LOCKING | DIO_SKIP_HOLES);
}
